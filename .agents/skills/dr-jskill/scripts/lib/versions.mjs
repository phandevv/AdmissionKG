 ...
 * (see references/TEST.md "Silent failure trap"). Idempotent.
 */
export function patchPomFailsafe(projectDir) {
  const pomPath = join(projectDir, 'pom.xml');
  if (!existsSync(pomPath)) return;
  let pom = readFileSync(pomPath, 'utf8');
  if (pom.includes('maven-failsafe-plugin')) return; // Already present
  const closingPlugins = pom.indexOf('</plugins>');
  if (closingPlugins === -1) return;
  const snippet =
    '\t\t\t<plugin>\n' +
    '\t\t\t\t<groupId>org.apache.maven.plugins</groupId>\n' +
    '\t\t\t\t<artifactId>maven-failsafe-plugin</artifactId>\n' +
    '\t\t\t</plugin>\n\t\t';
  pom = pom.slice(0, closingPlugins) + snippet + pom.slice(closingPlugins);
  writeFileSync(pomPath, pom, 'utf8');
}
